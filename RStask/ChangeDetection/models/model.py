...
class SimplifiedScaledDotProductAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None):
        ...
        # 线性变换并分割成多头
        Q = self.fc_q(queries).view(batch_size, -1, self.h, self.d_k).transpose(1, 2)  # (B, h, seq_len, d_k)
        K = self.fc_k(keys).view(batch_size, -1, self.h, self.d_k).transpose(1, 2)
        V = self.fc_v(values).view(batch_size, -1, self.h, self.d_k).transpose(1, 2)
        ...
        # 应用注意力权重到值
        output = torch.matmul(attention_weights, V)  # (B, h, seq_len, d_k)
...
# 方案1: 直接使用change caption的简化版本
class TDE_SingleText(nn.Module):
    """用于处理单个change caption的文本解码器"""
    def __init__(self, out_dim=64):
        super(TDE_SingleText, self).__init__()
        self.dim = out_dim
        self.out_d = out_dim
        self.conv = nn.Sequential(
            nn.Conv2d(self.out_d, self.out_d, kernel_size=3, stride=1, padding=1),
        ...
        # 使用自注意力增强change caption特征
        self.att = SimplifiedScaledDotProductAttention(d_model=self.dim, h=8)
        self.Up_conv1 = UpconvNet()
        self.Up_conv2 = DownsampleNet()
        
    def forward(self, change_text):
        """
        Args:
            change_text: 变化描述的文本特征 [B, text_dim]
        Returns:
            t2, t3, t4, t5: 多尺度文本特征
        """
        B = change_text.shape[0]
        # Reshape文本特征
        text = change_text.reshape(B, -1, self.dim)
        
        # 使用自注意力增强特征
        text_enhanced = self.att(text, text, text)
        
        # 上采样到空间维度
        text_spatial = self.Up_conv1(text_enhanced)
        
        # 进一步处理
        t2 = self.conv(text_spatial)
        t3, t4, t5 = self.Up_conv2(t2)
        
        return t2, t3, t4, t5

# 方案3: 混合方案 - 结合图像差异和change caption
class TDE_Hybrid(nn.Module):
    """混合文本解码器: 同时利用图像差异特征和change caption"""
    def __init__(self, out_dim=64):
        super(TDE_Hybrid, self).__init__()
        self.dim = out_dim
        self.out_d = out_dim
        
        # Change caption特征处理
        self.change_att = SimplifiedScaledDotProductAttention(d_model=self.dim, h=8)
        self.change_proj = nn.Linear(self.dim, self.dim)
        
        # 图像差异引导的特征处理
        self.Up_conv1 = UpconvNet()
        self.Up_conv2 = DownsampleNet()
        
        # 融合change caption和空间特征
        self.fusion_conv = nn.Sequential(
            nn.Conv2d(self.out_d * 2, self.out_d, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(self.out_d),
            nn.ReLU(inplace=True)
        )
    ...
    def forward(self, change_text, textA=None, textB=None):
        """
        Args:
            change_text: 变化描述文本特征 [B, text_dim]
            textA, textB: 可选的前后时相文本（如果没有则为None）
        Returns:
            t2, t3, t4, t5: 多尺度文本特征
        """
        B = change_text.shape[0]
        ...
        # 处理change caption
        change_text = change_text.reshape(B, -1, self.dim)
        change_enhanced = self.change_att(change_text, change_text, change_text)
        change_enhanced = self.change_proj(change_enhanced)
        
        # 上采样到空间维度
        change_spatial = self.Up_conv1(change_enhanced)
        
        # 如果提供了前后时相文本，也利用它们的差异信息
        if textA is not None and textB is not None:
            textA = textA.reshape(B, -1, self.dim)
            textB = textB.reshape(B, -1, self.dim)
            # 计算时相差异
            temporal_diff = torch.abs(textA - textB)
            temporal_spatial = self.Up_conv1(temporal_diff)
            # 融合change caption和时相差异
            fused = torch.cat([change_spatial, temporal_spatial], dim=1)
            t2 = self.fusion_conv(fused)
        else:
            # 只使用change caption
            t2 = self.conv(change_spatial)
...
class BaseNet(nn.Module):
    def __init__(self, input_nc=3, output_nc=1, use_change_caption=False):
        super(BaseNet, self).__init__()
        self.backbone = Resnet50()
        channles = [16, 24, 32, 96, 320]
        self.en_d = 32
        self.mid_d = self.en_d * 2
        self.use_change_caption = use_change_caption
        ...
        # 根据配置选择不同的文本编码器
        if use_change_caption:
            # 使用单文本的TDE（方案1）
            self.TDE = TDE_SingleText(self.en_d*2)
        else:
            # 使用原始的双文本TDE
    ...
    def forward(self, x1, x2, text_A, text_B=None):
        ...
        if self.use_change_caption:
            # 使用change caption（text_A作为change caption）
            t2,t3,t4,t5 = self.TDE(text_A)
        else:
            # 使用原始的双文本方式
...
# 方案3: 使用混合TDE的模型
class BaseNet_Hybrid(nn.Module):
    def __init__(self, input_nc=3, output_nc=1):
        super(BaseNet_Hybrid, self).__init__()
        self.backbone = Resnet50()
        channles = [16, 24, 32, 96, 320]
        self.en_d = 32
        self.mid_d = self.en_d * 2
        ...
        self.Image_encoder = Image_encoder()
        self.IFR = IFR(self.mid_d, self.en_d * 2)
        self.TDE = TDE_Hybrid(self.en_d*2)
        self.ITFF = ITFF(self.en_d * 2)
        
    def forward(self, x1, x2, change_text, text_A=None, text_B=None):
        """
        Args:
            x1, x2: 前后时相图像
            change_text: 变化描述文本特征
            text_A, text_B: 可选的前后时相独立描述
        """
        x1_2, x1_3, x1_4, x1_5, x2_2, x2_3, x2_4, x2_5 = self.Image_encoder(x1,x2)
        ...
        # 使用混合TDE
        t2,t3,t4,t5 = self.TDE(change_text, text_A, text_B)
